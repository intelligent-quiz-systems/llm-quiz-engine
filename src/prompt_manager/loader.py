    ...
    for file_path in directory.glob("*.json"):
        try:
            ...
            if not isinstance(data, dict):
                print(f"[ERR] Błąd w pliku {file_path.name}: zawartość nie jest słownikiem")
                continue
            ...
            print(f"[ERR] Błąd składni JSON w pliku {file_path.name}: {e}")
        except Exception as e:
            print(f"[ERR] Nieznany błąd podczas wczytywania {file_path.name}: {e}")
